...
const useSwipeCard = ({isFront, onRightSwipe, onLeftSwipe }:{isFront?: boolean, onRightSwipe?: () => void, onLeftSwipe?:  () => void }) => {
    ...
    const backgroundColor = position.x.interpolate({
      inputRange: [-30, 0, 30],
      outputRange: ['rgba(147, 142, 144, 0.7)', 'transparent', 'rgba(255, 177, 199, 0.7)'],
      extrapolate: 'clamp',
    });
    ...
    const animatedCardStyle = {
        transform: [
          ...
          { rotate },
          { scale: transition.interpolate({ inputRange: [0, 1], outputRange: [0.9, 1] }) },
